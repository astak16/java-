    ...
    getLatestTime() {
      ...
      if (countdownType === "normal") {
        ...
        if (timeType !== "second") {
          countDownTime =
            typeof time === "string"
              ? countDownTime.replace(/-/g, "/")
              : countDownTime;
          countDownTime = Math.ceil(
            (new Date(countDownTime).getTime() - new Date().getTime()) / 1000
          );
        }
        ...
        if (timeType === "second") {
          //  纪念日模式不能设置timeType === second
          console.error(
            `countdownType为${countdownType}类型时，不可设置timeType值为second`
          );
        } else {
          countDownTime =
            typeof time === "string"
              ...
              : countDownTime;
          countDownTime = Math.ceil(
            (new Date().getTime() - new Date(countDownTime).getTime()) / 1000
          );
          if (countDownTime >= 0) {
            //  countDownTime计算结果不能为负数
            this.getLatestForCountDown(countDownTime);
            this.init.call(this);
          } else {
